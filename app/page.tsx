...
export default async function HomePage() {
  ...
  const [{ rows: officeRows }, { rows: xpRows }] = await Promise.all([
    sql<OfficeRow>`
      ...
      GROUP BY o.name, o.slug, o.unlock_xp, o.description, o.sort_order
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {officeRows.map((office) => {
              const unlocked = totalXp >= office.unlock_xp;
              const pct = office.total_count > 0
                ? Math.round((office.met_count / office.total_count) * 100)
                : 0;
              const unlockPct = office.unlock_xp > 0
                ? Math.min(100, Math.round((totalXp / office.unlock_xp) * 100))
                : 100;
              ...
              return (
                ...
                >
                  ...
                  <p className="text-xs text-gray-400">Unlock at {office.unlock_xp} XP</p>
                  ...
                  <p className="text-xs text-gray-400">{totalXp} / {office.unlock_xp} XP</p>
                </div>
              );
            })}
          </div>
