...
export default async function OfficePage({
  ...
  const { rows: officeRows } = await sql<OfficeRecord>`
    SELECT name, slug, unlock_xp, description
    FROM offices
    WHERE slug = ${slug}
    LIMIT 1
  `;
  ...
  // Global XP needed to enforce the unlock gate
  ...
  if (totalXp < office.unlock_xp) redirect("/");
  ...
  // Next unlockable office, if any (to show progress nudge)
  const { rows: nextRows } = await sql`
    SELECT name, unlock_xp
    FROM offices
    WHERE unlock_xp > ${totalXp}
    ORDER BY unlock_xp
    LIMIT 1
  `;
  const nextOffice = nextRows[0] as { name: string; unlock_xp: number } | undefined;
    ...
    <main className="p-8 max-w-6xl mx-auto">
      ...
      {total === 0 ? (
        ...
        <>
          ...
          {/* Next office unlock nudge */}
          {nextOffice && (
            <div className="mb-6 flex items-center gap-3 px-4 py-3 bg-amber-50 border border-amber-200 rounded-xl text-sm text-amber-700">
              <span>🔒</span>
              <span>
                <strong>{nextOffice.name}</strong> unlocks at {nextOffice.unlock_xp} XP
                — you need {nextOffice.unlock_xp - totalXp} more.
              </span>
            </div>
          )}
